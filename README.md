...
- **AI 슬라이드 이미지 생성**: Gemini 이미지 모델로 슬라이드 배경, 레이아웃, 텍스트를 포함한 완성된 슬라이드 이미지를 자동 생성 (재생성 시 "명사형 종결", "배경 파란색" 등 사용자 지시 입력 가능)
- **부분 수정 (Partial Edit)**: 생성된 슬라이드에 자연어 수정 지시를 입력하면 AI가 해당 부분만 수정 (영역 선택 지원: 수정할 부분을 드래그로 지정 가능, 수정 지시를 큐에 담아 최대 3건 일괄 순차 처리)
...
- **초안 마크다운 다운로드**: 초안 미리보기 단계에서 생성된 슬라이드 콘텐츠를 `.md` 파일로 다운로드
