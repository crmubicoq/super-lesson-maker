...
- **[수정 내용]**:
  1. `SlideEditor.tsx`: 재생성 버튼 왼쪽에 텍스트 입력란 추가 (Enter키로도 재생성 가능)
  2. `handleRegenerate`에서 `customInstruction` 파라미터를 API에 전달
  3. `generate-slide-image/route.ts`: `SlideImageRequest`에 `customInstruction` 필드 추가
  4. `buildSlidePrompt()`에 "사용자 추가 지시" 섹션 조건부 추가 — 콘텐츠 원문은 유지하되 디자인/형식만 지시 반영
- **[핵심 원리]**: 지시문은 프롬프트의 별도 섹션("사용자 추가 지시")으로 전달되어, VERBATIM 콘텐츠와 분리됨. 비워두면 기존과 동일하게 동작.
- **[빌드 검증]**: 통과

### [진행 내용]: 슬라이드 이미지 텍스트 충실도 개선 (v7.7)
- **[배경]**: 생성된 슬라이드 이미지의 텍스트가 좌측 원본 대본과 다른 경우가 빈번. 원본에 없는 내용이 추가되거나 내용이 요약/변형되는 Content Drift 현상.
- **[원인]**: 기존 프롬프트가 콘텐츠를 자유 형식으로 전달하여 Gemini 이미지 모델이 "참고 자료"로 인식하고 자유롭게 재구성.
- **[수정 내용]**:
  1. 콘텐츠 섹션을 VERBATIM 블록으로 변경 — 각 텍스트를 따옴표(`""`)로 감싸 원문임을 명시
  2. 텍스트 체크리스트 섹션 추가 — 이미지에 반드시 존재해야 할 텍스트를 번호 매겨 나열 + "이 목록 외 추가 텍스트 삽입 금지"
  3. 절대 금지 사항 섹션 추가 — Content Drift 방지 규칙 5개 (텍스트 추가, 요약, 임의 삽입, 번호/부제목 생성, 원문 변경 금지)
- **[수정 파일]**: `src/app/api/generate-slide-image/route.ts` — `buildSlidePrompt()` 함수
- **[핵심 원리]**: 이미지 생성 모델은 자유 텍스트를 "재해석"하는 경향이 있음. 따옴표 + 체크리스트 + 금지 목록의 3중 장치로 원문 충실도를 극대화.
- **[빌드 검증]**: 통과
...
- **[수정 내용]**:
  1. "+큐에 추가 (N/3)" 버튼을 항상 표시 (텍스트 미입력 시 disabled 상태)
  2. 버튼 레이아웃을 가로(flex) → 세로(space-y)로 변경하여 가시성 향상
  3. "수정 지시" 라벨에 "최대 3건 일괄" 힌트 텍스트 추가
- **[수정 파일]**: `src/components/SlideEditor.tsx`
- **[빌드 검증]**: 통과
...
- 내보내기 섹션 신설 (저장되는 파일 목록 + 개별 다운로드 안내)

### [진행 내용]: 슬라이드 개별 다운로드 기능
- **[배경]**: 팀원 요청 — "슬라이드 저장" 버튼이 전체 슬라이드를 서버에 저장하는 동작이었는데, 현재 보고 있는 슬라이드 1장만 PNG로 PC에 다운로드하고 싶다는 요구.
- **[변경]**:
  - `SlideEditor.tsx`에 `downloadCurrentSlide()` 함수 추가 (html-to-image 캡처 → `<a download>` 브라우저 다운로드)
  - 버튼 텍스트: "슬라이드 저장" → "슬라이드 다운로드", onClick 핸들러 변경
  - 기존 `saveAllSlides()`는 "편집 완료 및 다음 단계" 버튼에서 그대로 사용 (영향 없음)
- **[파일명 형식]**: `slide_01.png`, `slide_02.png` (현재 슬라이드 번호 기준)
- **[빌드 검증]**: 통과

## 📅 2026-03-16
### [진행 내용]: start.bat Node.js 자동 탐색 + 포터블 다운로드 기능
- **[배경]**: 팀원 PC에서 `start.bat` 실행 시 "'npm' is not recognized" 오류 발생. Node.js가 설치되어 있었지만 관리자 모드 CMD에서 PATH가 달라 인식 못함.
- **[변경]**:
  - 4단계 탐색 로직: ① 시스템 PATH → ② 로컬 포터블 Node.js → ③ 일반 설치 경로 자동 탐색 (Program Files, AppData, nvm, fnm, scoop) → ④ 포터블 Node.js 자동 다운로드
  - 관리자 권한 실행 시에도 사용자 설치 Node.js를 찾을 수 있음
  - Node.js 미설치 환경: v22.14.0 LTS 자동 다운로드 + 압축 해제
  - `.gitignore`에 `/node/`, `node_portable.zip`, 로그/zip 파일 추가
- **[핵심 원리]**: Windows CMD에서 `where` 명령은 시스템 PATH만 검색. 관리자 CMD는 사용자별 PATH를 상속하지 않을 수 있으므로, 알려진 설치 경로를 직접 탐색하는 것이 안정적.
- **[빌드 검증]**: 통과
